...
<div class="space-y-6">
    ...
    @php
        $pendingOrders = $recentOrders->where('status', 'pending');
    @endphp
        ...
        <div class="flex items-center justify-between">
            <div class="flex items-center">
                <div class="flex-shrink-0">
                    <i data-lucide="bell" class="h-5 w-5 text-blue-500 animate-bounce"></i>
                </div>
                <div class="ml-3">
                    <p class="text-sm text-blue-700">
                        Ada <span class="font-bold">{{ $pendingOrders->count() }} pesanan baru</span> masuk! Segera proses sebelum konsumen datang.
                    </p>
                </div>
            </div>
            <a href="{{ route('mitra.orders') }}" class="text-sm font-bold text-blue-700 hover:underline">
                Lihat Semua
            </a>
        </div>
    </div>
            ...
            <div class="flex flex-col sm:flex-row sm:items-center justify-between p-4 border border-gray-100 rounded-xl gap-4 hover:border-green-200 transition">
                ...
                <div class="flex items-center gap-6">
                    <div class="text-right">
                        ...
                        <div class="text-xs mt-1 {{ $order->status === 'completed' ? 'text-green-600' : 'text-orange-600 font-bold' }}">
                            {{ ucfirst($order->status) }}
                        </div>
                    </div>
                    <div class="flex items-center gap-2">
                        @if($order->status === 'pending')
                        <form action="{{ route('mitra.orders.confirm', $order->id) }}" method="POST" class="inline">
                            @csrf
                            <button type="submit" class="bg-green-600 text-white px-4 py-2 rounded-lg text-sm font-bold hover:bg-green-700 transition">
                                Konfirmasi
                            </button>
                        </form>
                        @endif
                        <a href="{{ route('mitra.orders') }}" class="bg-gray-50 text-gray-600 px-4 py-2 rounded-lg text-sm font-bold hover:bg-green-50 hover:text-[#174413] transition border border-gray-100">
                            Detail
                        </a>
                    </div>
                </div>
